...
int main()
{
  ...
  // a P controller for the throttle
  pid_speed.Init(0.1, 0.0, 0.0);
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // correct steering_value
          if ( steer_value < STEER_MIN ) {
          ...
          double targetSpeed = 30.*(1.-fabs(steer_value)) + 30.;

          // compute the throttle
          pid_speed.UpdateError(speed-targetSpeed);
          throttle = -pid_speed.TotalError();
          // DEBUG
          std::cout << "cte:" << cte << ",steering:" << steer_value
        		  << ",speed:"<<  speed << ",target: " <<  targetSpeed  << ",tt:" <<  throttle << std::endl;
